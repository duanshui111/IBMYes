...
点击创建之后，进入项目，接着进入右上角命令行，进入后注意修改右上角地区必须是达拉斯
...
打开项目下的`fping-msys2.0`目录运行`自动查找最优CF节点-懒人专用.bat`（脚本不能直接运行，没有自动换行，用word打开一些再粘贴到bat运行；需要自行下载crul，放到同一目录）
